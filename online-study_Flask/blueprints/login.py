...
def register():   # 注册视图函数
    if request.method == "GET":
        ...
        try:
            ...
            msg=Message(subject='懒人tool网站验证码发送',body=f'验证码:{temp_password},谢谢您注册懒人tool在线学习平台,望您使用开心',
                recipients=[email])
            mail.send(msg)
            r=request.form.put('register_password')
                #发送邮件
        except:
           return render_template('/login/error.html')
        ...
        temp_password=request.args.get('register_password')
        print(user_temp_password)
        print(temp_password)
