...
function post_pic12(f) {
    ...
    if (rd == 0)
        ...
        $("#display_none24").show();
        rd.onloadend = function (e) {
            $("#display_none24").html("<img src='" + this.result + "' style=\"max-width:250px;max-height:250px;\"/>")
            $.ajax({
                url: "/imageprocessing/A13/",
                method: "POST",
                data: {
                    'data': this.result
                },
                success: function (result) {
                    ...
                    if (code == 200) {
                        $("#display_none25").show();
                        //加载完毕之后，在div中添加一个元素
                        $("#display_none25").html("<img src='data:image/jpeg;base64," + result["datas"] + "' style=\"max-width:250px;max-height:250px;\"/>")
                    } else {
                        alert("发送异常!");
                    }
                },
...
function post_pic13(f) {
    ...
    if (rd == 0)
        ...
        $("#display_none26").show();
        rd.onloadend = function (e) {
            $("#display_none26").html("<img src='" + this.result + "' style=\"max-width:250px;max-height:250px;\"/>")
            $.ajax({
                url: "/imageprocessing/A14/",
                method: "POST",
                data: {
                    'data': this.result
                },
                success: function (result) {
                    ...
                    if (code == 200) {
                        $("#display_none27").show();
                        //加载完毕之后，在div中添加一个元素
                        $("#display_none27").html("<img src='data:image/jpeg;base64," + result["datas"] + "' style=\"max-width:250px;max-height:250px;\"/>")
                    } else {
                        alert("发送异常!");
                    }
                },
